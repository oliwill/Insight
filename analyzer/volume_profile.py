    ...
    def _classify_regime(self, df: pd.DataFrame) -> VolumeProfile:
        ...
        if len(close) >= VOLUME_MID_PERIOD + 1:
            price_change_20d = float(
                (close.iloc[-1] / close.iloc[-1 - VOLUME_MID_PERIOD] - 1) * 100
            )
        else:
        ...
        baseline_volatility = (
            float(intraday_range.mean())
            if len(intraday_range) > 0
        ...
        spike_mask = (
            (volume >= VOLUME_SPIKE_MULTIPLIER * vol_20d_full) & valid_vol
        ).fillna(False)
        # 只统计最近 VOLUME_MID_PERIOD 个交易日内的爆量；更早期的历史爆量与当前形态无关，
        # 否则一次几年前的巨量+回落会把当前 regime 误判为「爆冲巨量」。
        recent_spike_mask = spike_mask.copy()
        if len(recent_spike_mask) > VOLUME_MID_PERIOD:
            recent_spike_mask.iloc[:-VOLUME_MID_PERIOD] = False
        is_volume_spike = bool(recent_spike_mask.any())

        spike_followed_by_decline = False
        if is_volume_spike:
            for idx in volume[recent_spike_mask].index:
        ...
        if is_volume_spike and spike_followed_by_decline:
            ...
            profile.risks.append(
                "爆冲巨量后走阴跌，疑似抢帽子出货/主力派发（趋势博弈框架四问法：等量能平静后再看）"
            )
        elif is_flat:
            profile.regime = "平量推升"
            profile.signals.append(
                "平量推升：筹码锁定，最优质走法（趋势博弈分析框架：好东西大家都不舍得送给别人）"
            )
        elif price_change_20d > 3.0 and vol_ratio > 1.2 and price_vol_corr > 0:
            profile.regime = "量价齐升"
            profile.signals.append(
                "量价齐升，确定性较高（注意：最连贯的一段往往在天量之前）"
            )
