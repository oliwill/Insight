...
# 同行基准（预计算）
PEER_BENCH = {
    'SH688035': {'ps_med': 7.1, 'pe_med': 75.7, 'top': '雅克科技(0.59)', 'detail': '雅克科技PS8.3/新益昌PS7.1/中国巨石PS8.0'},
    'SZ002468': {'ps_med': 0.5, 'pe_med': 13.9, 'top': '顺丰控股(0.41)', 'detail': '顺丰PS0.6/中国外运PS0.4'},
    'SH688106': {'ps_med': 7.1, 'pe_med': 75.7, 'top': '雅克科技(0.39)', 'detail': '特种气体独立性强，相关性低'},
    'SH688188': {'ps_med': 17.5, 'pe_med': 96.0, 'top': '汇川技术(0.48)', 'detail': '汇川PS3.6/北方华创PS13/中微PS27.5/华峰测控PS51'},
}
...
# 估值反向推导 + 赔率原型
# ============================================================
def valuation_reverse(ps, ps_peer_med, rev_growth, pe, eg, mcap, tech_bullish, bottom_signal):
    """返回 (透支倍数, 隐含年数, 安全边际, 赔率原型)"""
    overpay = ps / ps_peer_med if ps_peer_med > 0 else 1.0
    if overpay > 1 and rev_growth > 0:
        implied_years = math.log(overpay) / math.log(1 + rev_growth)
    elif overpay < 1:
        implied_years = -math.log(1 / overpay) / math.log(1 + max(rev_growth, 0.01)) if rev_growth > 0 else 0
    else:
        implied_years = 0
    # 安全边际
    if overpay < 0.8:
        ...
        margin = '无安全边际'
    # 赔率原型
    psg = ps / (rev_growth * 100) if ps > 0 and rev_growth > 0 else None
    if rev_growth > 0.2 and psg is not None and psg < 1.5 and tech_bullish:
        odds = '高确定性增长型'
    elif rev_growth > 0.2 and ((psg is not None and psg > 2) or not tech_bullish):
        odds = '催化剂驱动型'
...
# 三情景目标价
# ============================================================
def three_scenarios(price, ps, ps_peer_med, rev_growth, mcap, target_mean=0):
    """PS相对估值法"""
    ...
    bull = base * 1.3
    bear = base * 0.7
    weighted = bull * 0.25 + base * 0.50 + bear * 0.25
    implied_ret = (weighted / price - 1) * 100 if price > 0 else 0
    return bull, base, bear, weighted, implied_ret
...
def _has_metric(value):
    ...
    try:
        return value is not None and math.isfinite(float(value)) and abs(float(value)) > 1e-12
...
def _fundamental_rows(pe, pf, pb, gm, om, roe, rev_g, eg, cash, debt, debt_equity):
    ...
    rows = []
    if _has_metric(pe):
        rows.append(f"| PE(TTM) | {pe:.1f} | {'偏高' if pe > 50 else ('合理' if pe < 25 else '中等')} |")
    if _has_metric(pf):
        forward_note = '预期改善' if not _has_metric(pe) or pf < pe else '预期承压'
        rows.append(f"| PE(Forward) | {pf:.1f} | {forward_note} |")
    if _has_metric(pb):
        rows.append(f"| PB | {pb:.2f} | {'偏高' if pb > 4 else ('合理' if pb < 2 else '中等')} |")
    if _has_metric(gm):
        rows.append(f"| 毛利率 | {gm:.1f}% | {'优秀' if gm > 40 else ('中等' if gm > 20 else '偏低')} |")
    if _has_metric(om):
        rows.append(f"| 经营利润率 | {om:.1f}% | {'优秀' if om > 20 else ('中等' if om > 10 else '偏低')} |")
    if _has_metric(roe):
        rows.append(f"| ROE | {roe:.1f}% | {'优秀' if roe > 15 else ('中等' if roe > 8 else '偏低')} |")
    if _has_metric(rev_g):
        rows.append(f"| 营收增速 | {rev_g * 100:.1f}% | {'高增长' if rev_g > 0.2 else ('稳健' if rev_g > 0.1 else '疲软')} |")
    if _has_metric(eg):
        rows.append(f"| 盈利增速 | {eg:.1f}% | {'爆发' if eg > 50 else ('健康' if eg > 15 else '承压')} |")
    if _has_metric(cash) or _has_metric(debt):
        net_cash = (cash - debt) / 1e8
        rows.append(f"| 净现金 | {net_cash:+.1f}亿 | {'健康' if net_cash > 0 else '负债承压'} |")
    if _has_metric(debt_equity):
        # Yahoo's debtToEquity field is expressed as a percentage (e.g. 12.3 = 12.3%).
        rows.append(f"| 债务/权益 | {debt_equity:.1f}% | {'高杠杆' if debt_equity >= 100 else ('可控' if debt_equity < 60 else '需关注')} |")
...
def _growth_quality_lines(rev_g, eg, gm, cash, debt):
    lines = []
    if _has_metric(rev_g):
        lines.append(f"- {'✅ 营收增速健康' if rev_g > 0.15 else '⚠️ 营收增速疲软'}（{rev_g * 100:.1f}%）")
    if _has_metric(eg) and _has_metric(rev_g):
        lines.append('- ✅ 盈利增速匹配/超越营收' if eg > rev_g * 100 else '- ⚠️ 盈利增速落后于营收，利润率承压')
    if _has_metric(cash) or _has_metric(debt):
        lines.append('- ⚠️ 高负债模式，利率敏感' if cash < debt else '- ✅ 净现金充裕，抗风险强')
    if _has_metric(gm):
        if gm < 15:
...
def _moat_rows(gm, mcap):
    ...
    return "\n".join([
        ...
        f"| 客户锁定 | ★★★ | {'行业龙头地位' if mcap_available and mcap > 1e10 else '中小盘，客户集中度需关注'} |",
...
def _valuation_details(name, ps, pe, peer, overpay, impl_years, margin, odds, psg, rev_g, tech_bullish, bottom_signal):
...
> 数据缺口：未获取可验证的本股 PS 或同行 PS 中位数，跳过相对估值、PSG 与反向推导。"""
    ...
    psg_note = ('<1 合理' if psg < 1 else ('1-2 偏高' if psg < 2 else '>2 极度高估')) if psg is not None else '营收增速缺失，无法计算'
    growth_text = f"营收增速{rev_g * 100:.0f}%" if _has_metric(rev_g) else '营收增速未获取'
    return f"""### PS同行对比
| 标的 | PS | PE |
|------|-----|-----|
...
def trading_grid(price, period_low, period_high, atr, channel_lower, channel_upper, ma50, target, timing_state=''):
    """Build entries with per-entry ATR stops and unambiguous reward/risk values."""
    fib = {lvl: period_high - (period_high - period_low) * lvl for lvl in (0.236, 0.382, 0.5, 0.618, 0.786)}
    ...
    if 0 < deep < price:
        append_row('🔴 深度加仓', deep, '触及78.6%回撤或道氏通道下沿-5%；跌破当前ATR风险线则取消加仓', '30%')
    ...
    action = '可追' if timing_state == 'Ready' and target and target > price * 1.15 else '不追'
    current_trigger = '时机状态 Ready 且目标价空间≥15%' if action == '可追' else f'时机状态 {timing_state or "未确认"}，不在当前价追高'
...
def extract_alerts(p, c, f, m, rsi, price, ma20):
    ...
    if f.distribution_evidence >= 70:
        red.append(f'连续2日高位放量滞涨（量≥2×均量且涨幅<1%）→ 减仓，主力派发证据{f.distribution_evidence:.0f}')
    if rsi > 70:
        red.append(f'RSI 突破 70 → 减仓20%，超买区')
    if c.is_rubbing_upper:
        red.append(f'连续触碰通道上沿 ¥{c.upper_channel:.2f} 未突破≥3次 → 减仓，多方外强中干')
    if f.retail_trap_risk >= 70:
        red.append(f'连续涨停+小市值+爆量 → 警惕散户陷阱风险{f.retail_trap_risk:.0f}，趋势博弈框架四问法')
    # 🟢 加仓信号
    if p.is_flat_volume_uptrend:
        green.append(f'量比维持0.6-1.2 且20日涨幅>3% → 可加仓，平量推升筹码锁定')
    if '趋缓' in c.slope_state:
        green.append(f'下降通道斜率趋缓+缩量至通道下沿 ¥{c.lower_channel:.2f} → 可试探建仓（扶老太太下楼）')
    if c.top_bottom_signal == '底部三步信号':
        green.append(f'缩量筑底后放量站上通道下沿 ¥{c.lower_channel:.2f} → 可加仓，底部确认')
    if f.accumulation_evidence >= 65:
        green.append(f'底部连续长下影线+低点抬高 → 可加仓，主力吸筹证据{f.accumulation_evidence:.0f}')
...
def persist_to_obsidian(symbol: str, stock_name: str, report: str, title_date: str = '') -> None:
    """幂等写入完整分析，不覆盖已有股票页的其他研究和历史章节。"""
    from memory.manager import MemoryManager

    section_name = f"{stock_name} 完整版分析报告{title_date}" if title_date else REPORT_SECTION
...
# 报告生成主函数
# ============================================================
def generate(sym, name, yf_code, sector, industry, biz, *, output_dir: Path | None = None, write_to_obsidian: bool = False):
    ...
    rev_g = fund.get('revenue_growth', 0) or 0
    eg = (fund.get('earnings_growth', 0) or 0) * 100
    ...
    peer = PEER_BENCH.get(sym, {'ps_med': ps, 'pe_med': pe, 'top': 'N/A', 'detail': ''})
    ...
    period_low = float(close.tail(252).min()) if len(close) >= 252 else float(close.min())
    period_high = float(close.tail(252).max()) if len(close) >= 252 else float(close.max())

    # SEPA Stage
    if price > ma50 > ma120 > ma200 and ma200 > float(close.tail(200).head(1).iloc[0] if len(close) >= 200 else ma200):
    ...
    wk_support = float(wk_struct.support_level) if wk_struct and wk_struct.support_level else 0
    wk_resist = float(wk_struct.resistance_level) if wk_struct and wk_struct.resistance_level else 0
    wk_conf = (wk_struct.confidence * 100) if wk_struct else 0

    # 趋势博弈分析框架
    vp = _va.analyze(df.copy(), fund); p = vp.details['profile']
    dc = _dca.analyze(df.copy(), fund); c = dc.details['channel']
    fb = _fa.analyze(df.copy(), fund); f = fb.details['force_balance']
    mt = _ma.analyze(df.copy(), fund); m = mt.details['view']

    # Timing
    tech_bullish = (price > ma20) and (m.daily_trend == '上升')
    md = {
        'stock_info': {'price': price, 'name': name},
        'fundamentals': {'market_cap': mcap, 'pe_ttm': pe, 'pe_forward': pf, 'target_mean_price': target_mean},
        'technicals': {'trend_short': 'BULLISH' if price > ma20 else 'BEARISH', 'rsi_14': rsi,
                       'vol_ratio': vol_ratio, 'ma20': ma20, 'ma50': ma50, 'support_20d': float(df['low'].tail(20).min())},
        'volume_profile': {'regime': p.regime, 'is_volume_spike': p.is_volume_spike},
        'dow_channel': {'channel_direction': c.channel_direction, 'slope_state': c.slope_state,
                        'neckline_signal': c.neckline_signal, 'top_bottom_signal': c.top_bottom_signal,
                        'position_in_channel': c.position_in_channel, 'lower_channel': c.lower_channel,
                        'upper_channel': c.upper_channel},
        'force_balance': {'bull_bear_state': f.bull_bear_state, 'accumulation_evidence': f.accumulation_evidence,
                          'distribution_evidence': f.distribution_evidence, 'chip_lock_likelihood': f.chip_lock_likelihood,
                          'retail_trap_risk': f.retail_trap_risk},
        'multi_timeframe': {'alignment': m.alignment, 'higher_tf_signal': m.higher_tf_signal,
                            'daily_trend': m.daily_trend, 'weekly_trend': m.weekly_trend, 'monthly_trend': m.monthly_trend},
    }
    ...
    overpay, impl_years, margin, odds, psg = valuation_reverse(
        ps, peer['ps_med'], rev_g, pe, eg / 100, mcap, tech_bullish, c.top_bottom_signal)

    # 三情景
    bull_t, base_t, bear_t, weighted_t, implied_ret = three_scenarios(
        price, ps, peer['ps_med'], rev_g, mcap, target_mean)

    # 操作格网
    grid = trading_grid(price, period_low, period_high, atr, c.lower_channel, c.upper_channel, ma50, weighted_t, ts.state)
    ...
    risks = []
    if p.is_volume_spike:
        risks.append(('爆冲巨量出货风险', -15, -25, '高' if p.spike_followed_by_decline else '中'))
    if c.neckline_signal == '颈线已跌破':
        risks.append(('颈线破位趋势恶化', -10, -20, '高'))
    if overpay > 1.5:
        risks.append((f'估值泡沫（PS {ps:.1f} vs 同行{peer["ps_med"]:.1f}）', 0, -30, '中'))
    if net_cash < -30:
    ...
    if rev_g < 0.1:
        risks.append((f'营收增速疲软（{rev_g*100:.0f}%）', -8, -10, '中'))
    if not tech_bullish:
    ...
    verdict = '可关注/小仓位试探' if score >= 55 else ('观望' if score >= 40 else '回避')
    fundamental_rows = _fundamental_rows(pe, pf, pb, gm, om, roe, rev_g, eg, cash, debt, de_ratio)
    ...
    valuation_details = _valuation_details(
        name, ps, pe, peer, overpay, impl_years, margin, odds, psg, rev_g, tech_bullish, c.top_bottom_signal,
    )
    ...
    mcap_description = f'市值 ¥{mcap / 1e8:.0f}亿' if _has_metric(mcap) else '市值数据未获取'
...
published: {today}
created: {now_str}
description: "{name}：PSG {psg_description}，加权目标价 ¥{weighted_t:.2f}（{implied_ret:+.0f}%），{margin}"
tags:
  - stock-analysis
...
- **形态**：**{p.regime}** | 量比{p.vol_contraction_ratio:.2f} | 量价相关{p.price_vol_correlation:+.2f} | 20日涨幅{p.price_change_20d_pct:+.1f}%""")
    if p.is_volume_spike:
        L.append(f"  - ⚠️ **爆冲巨量**（后跌={p.spike_followed_by_decline}）：疑似抢帽子出货")
    if p.is_flat_volume_uptrend:
...
> 注：A股期权/做空/Reddit情绪数据暂不可用，仅显示流动性+多空博弈。""")
    ...
    # 流动性（简化）
    adv = vol_20d * price
    L.append(f"- **日均成交额**：¥{adv/1e8:.1f}亿（{'流动性充裕' if adv > 5e8 else '流动性一般' if adv > 1e8 else '流动性较差'}）")
    L.append(f"- **ATR**：¥{atr:.2f}（{atr/price*100:.1f}%，{'高波动' if atr/price > 0.04 else '低波动'}）")
...
| 情景 | 概率 | 12个月目标价 | 核心假设 |
|------|------|-------------|----------|
| 🟢 Bull | 25% | ¥{bull_t:.2f} | 估值扩张30%，增长超预期 |
| 🟡 Base | 50% | ¥{base_t:.2f} | {'分析师共识' if target_mean else 'PS回归同行中位数'} |
| 🔴 Bear | 25% | ¥{bear_t:.2f} | 估值收缩30%，增长不及预期 |

- **概率加权目标价**：**¥{weighted_t:.2f}**
- **隐含12个月回报**：**{implied_ret:+.1f}%**
- 当前价 ¥{price:.2f} vs 加权目标 ¥{weighted_t:.2f}
...
|------|------|---------|------|------|--------------|----------|""")
    for label, lvl_price, stop, trigger, pos, rr in grid:
        ...
        L.append(f"| {label} | ¥{lvl_price:.2f} | ¥{stop:.2f} | {label.split(' ')[1]} | {pos} | {rr_str} | {trigger} |")
...
| 时间 | 事件 | 超预期→ | 不及预期→ |
|------|------|---------|-----------|
| 季报期 | 下次财报 | 营收增速维持{rev_g*100:.0f}%+ → +10% | 增速下滑 → -15% |
...
if __name__ == '__main__':
    default_stocks = [
        ('SH688035', '德邦科技', '688035.SS', 'Basic Materials', 'Chemicals', '半导体封装材料'),
        ('SZ002468', '申通快递', '002468.SZ', 'Industrials', 'Integrated Freight & Logistics', '快递物流'),
        ('SH688106', '金宏气体', '688106.SS', 'Basic Materials', 'Specialty Chemicals', '特种气体'),
        ('SH688188', '柏楚电子', '688188.SS', 'Technology', 'Semiconductors', '激光控制系统/半导体设备'),
    ]
    parser = argparse.ArgumentParser(
        description='生成趋势博弈分析框架增强的完整报告；默认仅输出草稿，不覆盖 Obsidian 股票页。'
    )
    parser.add_argument('symbols', nargs='*', help='可选：SH688035 / 688035.SH / SZ002468 等')
    parser.add_argument('--write', action='store_true', help='将报告幂等更新到 Obsidian 的「趋势博弈分析框架完整分析」章节')
    parser.add_argument('--output-dir', type=Path, default=Path('tmp_analysis/full_reports'), help='独立 Markdown 草稿输出目录')
    ...
    for raw_symbol in args.symbols:
        ...
        if not normalized.startswith(('SH', 'SZ')):
            normalized = ('SH' if raw_symbol.upper().endswith('.SH') else 'SZ') + normalized
    ...
    for sym, name, yf_code, sector, industry, biz in stocks:
        result = generate(sym, name, yf_code, sector, industry, biz, output_dir=args.output_dir, write_to_obsidian=args.write)
        if result:
            score, chars = result
